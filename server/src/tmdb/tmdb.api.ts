...
const tmdbApi = {
    mediaList: async ({mediaType, mediaCategory, page}: MediaList) => await axiosClient.get(
        tmdbEndpoints.mediaList({mediaType, mediaCategory, page})
    ),
    mediaDetail: async ({mediaType, mediaId}: MediaDetail) => await axiosClient.get(
        tmdbEndpoints.mediaDetail({mediaType, mediaId})
    ),
    mediaGenres: async ({mediaType}: MediaBase) => await axiosClient.get(
        tmdbEndpoints.mediaGenres({mediaType})
    ),
    mediaCredits: async ({mediaType, mediaId}: MediaDetail) => await axiosClient.get(
        tmdbEndpoints.mediaCredits({mediaType, mediaId})
    ),
    mediaVideos: async ({mediaType, mediaId}: MediaDetail) => await axiosClient.get(
    ...
    personDetail: async ({personId}: PersonType) => axiosClient.get(
        tmdbEndpoints.personDetail({personId})
    ),
    personMedias: async({personId}: PersonType) => axiosClient.get(
